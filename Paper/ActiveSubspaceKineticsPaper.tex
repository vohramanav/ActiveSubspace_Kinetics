...
It was shown in~[ref] that the total Sobol' index $T_i(f)$ can be bounded
in terms of $\nu_i(f)$:
\[
...
Of course, if $\xi_i$ are iid, the $C_i$'s will cancel and the normalized screening
metric reduces to 
\[
    \tilde{\nu}_i(f) = \frac{\nu_i(f)}{\sum_{i=1}^\Np \nu_i(f)} 
...
\item $\displaystyle
\tilde{\nu}_i(f) \leq \frac{ C_i \big(\alpha_i(f; r) + \lambda_{r+1})}{\sum_{i=1}^\Np C_i \alpha_i(f; r)}$.
\end{enumerate}
\end{proposition}

In the case where $\xi_i$ are iid and where $\lambda_{r+1} \approx 0$, 
