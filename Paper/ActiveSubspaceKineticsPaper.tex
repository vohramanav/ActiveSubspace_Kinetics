...
\nu_j(f) = \vec{e}_j^T \Big(\sum_{k=1}^\Np \lambda_k \vec{u}_k \vec{u}_k^T\Big) \vec{e}_j
 = \sum_{k=1}^\Np \lambda_k \ip{\vec{e}_j}{\vec{u}_k}^2, 
\end{equation}
...
\begin{proposition} For $1 \leq r \leq \Np$,
\[
0 \leq \nu_j(f) - \alpha_j(f; r) \leq \lambda_{r+1},
\] 
for $j=1, \ldots, \Np$.
\end{proposition} 
...
\]
Generalizing the result of~\cite{Diaz16}, we present
the following proposition linking activity scores and Sobol' indices.
\begin{proposition}
...
\leq \kappa 
\frac{ \alpha_i(f; r) + \lambda_{r+1}}{\sum_{i=1}^\Np \alpha_i(f; r)}$, with 
$\kappa = \displaystyle\frac{\max_i C_i}{\min_i C_i}$.
\end{enumerate}
\end{proposition}

In the case where where $\lambda_{r+1} \approx 0$, 
the above result motivate definition of
normalized activity scores
\[
   \tilde{\alpha}_i(f; r) =  \frac{\alpha_i(f; r)}{\sum_{i=1}^\Np \alpha_i(f; r)}.
\] 
