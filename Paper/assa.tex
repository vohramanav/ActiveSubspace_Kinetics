...
\]
Following~\cite{Constantine:2015}, we use the approximation $f(\vec{\xi}) \approx 
...
\]
The expression for the denominator follows, since 
$\sum_{i=1}^\Np \nu_i(f) = \trace(\mat{C}) = \sum_{k = 1}^\Np \lambda_k$.
Also, in the case of iid inputs, the normalized activity scores reduce to
\[
