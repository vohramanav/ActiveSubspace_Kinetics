...
Mathematically, these directions are the eigenvectors of a positive
semi-definite matrix, $\mat{C}$, defined as follows: (\alennote{Need to 
say what is the function $f$, first})
%
\be
...
\ee
%
where $\Omega$:~$[-1,1]^{N_p}$ is the domain of $\bm{\xi}$, and $N_p$ is the
number of uncertain inputs. \alennote{Why do we limit to $[-1, 1]^\Np$?} 
...
\ee
%
where $\bm{\Lambda}$ = diag($\lambda_1,\ldots,\lambda_{N_p}$) with
...
\ee
 %
 where $\bm{w}_1$ comprises the dominant eigenspace of $\mat{C}$ regarded as
the active subspace, and $\bm{\Lambda}_1$ is the corresponding set of
eigenvalues. 
\alennote{$\vec{w}_1$ is a supposedly the matrix whose columns are 
the dominant eigenvectors. Denoting a matrix by a notation typically 
used for a vector is confusing. Use $\mat{W}_1$ instead?}
...
variables, $\bm{w}_1^\top \bm{\xi}$ and $f(\bm{\xi}(\theta))$ is transformed as
$G(\bm{w}_1^\top \bm{\xi})$. In practice, the set of samples, $\bm{\xi}_i$'s
...
unimportant parameters at nominal values. 
...
The \emph{reduced} model, obtained by fixing unimportant variables is given by
\newcommand{\xin}{\vec{\xi}^{(\sim i)}}
\[
f_{z}(\vec\xi) = f(\xin, z)
\] 
where $(\vec{y}, \vec{z})$ should be understood as a vector $\vec\xi$ with
$\vec\xi_{I^c}$ set to $\vec{y}$ and $\vec\xi_{\I}$ set to $\vec{z}$.
The following result quantifies the approximation error when using $f_\vec{z}$
in place of $f$:
\begin{theorem}
Consider the relative error
\[
\delta(z) =
\frac{ \int_\Omega \big( f(\vec\xi) - f_{z}(\vec\xi)\big)^2 \, \mu(d\vec\xi) }
          {\int_\Omega \big( f(\vec\xi) )^2 \, \mu(d\vec\xi)}.
\] 
We have
\[
          \Ez{ \delta(z)} \leq \frac{2 C_i}{\V(f)}\big(\act{f}{i}{r} + \lambda_{r+1}\big).
\]
\end{theorem}
%
%\[
