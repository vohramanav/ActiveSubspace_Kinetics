...
The directions constituting the active subspace are the eigenvectors of the positive
semidefinite matrix 
%
\be
...
We also record the following useful relations:
...
In~\cite{Vohra:2018} the following screening metric
\be
...
\[
   \actt{f}{i}{r} =  \frac{\act{f}{i}{r}}{\sum_{i=1}^\Np \act{f}{i}{r}} = 
                     \frac{\sum_{k=1}^r \lambda_k \ip{\vec{e}_i}{\vec{w}_k}^2}
                          {\sum_{k = 1}^r \lambda_k}.
\]
%The expression for the denominator can be seen from Lemma~\ref{lem:sum}(a). 
