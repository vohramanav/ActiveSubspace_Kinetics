...
\[\int_\Omega \big( f(\vec\xi) - f^{z}(\vec\xi)\big)^2 \, \mu(d\vec\xi) = 2\V_{i}^{tot}(f)  \] where 
\[\V{i}^{tot}(f) = \V_i(f) + \V_{i,j}(f) \]
...
\[ \Ez{ \delta(z)} = 2 \frac{\V_{i}^\text{tot}(f)}{\V(f)} = 2\bigg(1 - \frac{\V_j(f)}{\V(f)}\bigg) = 2T_{i}(f)\]
