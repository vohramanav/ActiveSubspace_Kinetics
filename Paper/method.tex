...
that involves a 
local linear approximation of
the model output to circumvent the computational cost associated with gradient
computation. 
(\alennote{should we say regression-based local linear approximation? 
When I hear local linear approximation I think of Taylor expansion, and
probably many readers will do the same; so saying gradient-free
local linear approximation might be confusing to some people.}) 
...
problems~\cite{Constantine:2015}.~\alennote{I would not say this. 
If Paul (or a student of his) 
reviews this article, he will probably disagree. Remember that 
book was from several years ago. Instead, say that we 
demonstrate its utility for efficient response surface construction in 
chemical kinetics modeling.} We are motivated by the
