...
\Ensure $\Lambda$, $W$, $\alpha$ %$\eta$. %
	...
	\State Compute $\mat{C}$ and its eigenvalue decomposition 
		$\mat{C}$= $\frac{1}{N_\text{total}}\sum\limits_{i=1}^{N_\text{total}}[\bm{g}^i][\bm{g}^i]^\top = W^{(k)}\Lambda^{(k)} W^{(k)\top}$
	...
	\State Partition: $\Lambda^{(k)}=
        ...
        $\Lambda_1^{(k)}\in \mathbb{R}^{N_p\times\mathcal{S}}$
	\Loop
		\State Set $k$ = $k$ + 1
		\State Draw $n_k =  \beta n_{k-1}$  new random samples 
                $\{\bm{\xi}_i\}_{i=1}^{n_k}$  $\beta =??$  $k = n_{r-1}+1,\ldots,n_{r-1}+n_r$.
	...
	%	\State Project $\bm{\xi}_k$~$\rightarrow$~$\bm{\theta}_k$.%
		...
		\State Compute $\bm{g}^i = \nabla_{\bm{\xi}_i}f(\bm{\xi}_i)$, 
             	$i=n_{r-1}+1, \ldots, n_{r-1}+n_r$.  
		\State Compute $\mat{C}$ = 
        	$\frac{1}{N_\text{total}}\sum\limits_{k=1}^{N_\text{total}}[\bm{g}^i][\bm{g}^i]^\top$
		\State Eigenvalue decomposition, $\mat{C}$ = $W^{(k)}\Lambda^{(k)} W^{(k)\top}$
