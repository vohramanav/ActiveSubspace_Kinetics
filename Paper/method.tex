...
\Ensure $\Lambda$, $W$, $\eta$. 
	...
	\State Compute $\mat{C}$ and its eigenvalue decomposition 
		$\mat{C}$= $\frac{1}{N_\text{total}}\sum\limits_{k=1}^{N_\text{total}}[\bm{g}^k][\bm{g}^k]^\top = W^{(0)}\Lambda^{(0)} W^{(0)\top}$
	...
	\State Partition the eigenpairs: $\Lambda^{(0)}~=~ 
        ...
        $\Lambda_1^{(0)}\in \mathbb{R}^{N_p\times\mathcal{S}}$ \hayleynote{What is S?}
...
\Ensure $\Lambda$, $W$, $\eta$. 
	...
	\Loop
		\State Set $r$ = $r$ + 1
		\State Draw $n_r$ \iffalse = $\lceil \beta n_1 \rceil$ \fi new random samples 
                $\{\bm{\xi}_k\}_{k=1}^{n_r}$ $\in$ [-1,1] \iffalse $\beta = 2,\ldots $ $k = n_{r-1}+1,\ldots,n_{r-1}+n_r$. \fi
        \State $N_\text{total}$ = $N_\text{total}$ + $n_r$   
        \State Repeat steps 5-9  
