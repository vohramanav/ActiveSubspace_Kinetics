...
\Ensure $\bm{\hat{\Lambda}}$, $\bm{\hat{W}}$, $\bm{\alpha}$. 
	...
	\Loop
		...
		\State Compute $\bm{\delta w_{1,j}^{(r)}}$ = 
                       \scalebox{1.25}{$\frac{\|(\bm{w_{1,j}^{r}})^2 - 
                       (\bm{w_{1,j}^{r-1}})^2\|}{\|(\bm{w_{1,j}^{r-1}})^2\|}$}, 
                       $j = 1,\ldots,\mathcal{S}$.
		\If {$\max\left(\bm{\delta w_{1,j}^{(r)}}\right)<\tau$}
			\State break
		\EndIf
	\EndLoop
	\State Compute $\alpha_i(N_\text{total}) = \sum\limits_{j=1}^{\mathcal{S}} \hat{\Lambda}_{1_{i,j}}w_{1_{i,j}}^2$,
	$i=1,\ldots,d$.
