...
gradients in order to construct the matrix (gradient-based), $\hat{\mat{C}}$ in~\eqref{eq:chat}, and the other that involves a local 
linear approximation of the model output to circumvent the computational cost associated with gradient
computation (gradient-free). The gradient-based approach is based on Algorithm 1.1, and the gradient-free approach is
...
Since, the gradient-free approach is relatively more efficient, this observation encourages its use for high-dimensional
...
As discussed earlier, in the gradient-based approach, the elements of $\hat{\mat{C}}$ are estimated using model gradients.
...
estimate of the dominant eigenspace, $\hat{\mat{W}}_1$ and corresponding set of eigenvalues, $\hat{\mat{\Lambda}}_1$.
...
highest relative change at iteration $k$, $\delta \hat{\mat{W}}_{1,j}^{(k)}$, is smaller a given tolerance, $\tau$. 
A regression fit to
 $G(\hat{\mat{W}}_1^\top\bm{\xi})$ is used as a surrogate to characterize and quantify the uncertainty in the model
 output. Moreover, the components of the dominant eigenvectors are used to compute the activity scores,
 $\bm{\nu}_r(f)$, which provide an insight into the relative importance of the uncertain inputs. Note that the
 index, $r$, corresponds to the number of dominant eigenvectors in $\hat{\mat{W}}_1$. The sequence of 
 steps for the gradient-based approach are provided in Algorithm~\ref{alg:grad}.
...
the comparison of the resulting normalized eigenvalue spectrum ($\lambda_1 - \lambda_{19}$) with the same using a much 
...
the higher-order eigenvectors. In this case, since $\left(\frac{\lambda_1}{\lambda_2}\right)\gg 1$, we have
...
As mentioned earlier, the gradient-free approach does not involve computation of the gradient of the model
output. Instead, a least-squares fit is performed to the local linear regression-based approximation
to the model output. While the gradient-free approach offers a significant potential for additional 
...
the components of the dominant eigenvector as shown in Algorithm~\ref{alg:grad}. The activity scores for the 19 
uncertain pre-exponents ($A_i$'s), estimated using
...
effort in the case of gradient-free approach is significantly smaller. Hence, we are encouraged to use the
gradient-free approach for the 33-dimensional application in the next section. 
