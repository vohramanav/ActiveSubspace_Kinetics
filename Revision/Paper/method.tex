...
\Ensure $\hat{\mat{\Lambda}}$, $\hat{\mat{W}}$, $\bm{\nu}_r(f)$ %$\eta$. %
	...
	%	\State Project $\bm{\xi}_k$~$\rightarrow$~$\bm{\theta}_k$.%
		...
		\State Compute $\delta \hat{\mat{W}}_{1,j}^{(k)}$ = 
                       ...
                       (\hat{\mat{W}}_{1,j}^{k-1})^2\|_\rebut{2}}{\|(\hat{\mat{W}}_{1,j}^{k-1})^2\|_\rebut{2}}$}, 
                       $j = 1,\ldots,r$.
		\If {$\max\limits_{\rebut{j}}\left(\delta \hat{\mat{W}}_{1,j}^{(k)}\right)<\tau$}
			\State break
		\EndIf
	\EndLoop
...
problem by perturbing \rebut{$\log(A_i)$ by 3$\%$ about its nominal
...
are approximated 
reasonably well with just 20 samples. As expected, the accuracy of higher-\rebut{index}
...
for a test case involving $N$~=~1000 (and $M$ = 20,000 model runs)
samples using the perturbation approach are also plotted. 
...
approaches in this case. \rebut{Since both approaches yield consistent results and are
