...
problem whereby the \rebut{logarithm of the} 
...
distributed in the interval, \rebut{$[0.97\log(A_i^\ast), 1.03\log(A_i^\ast)]$};
...
\rebut{$\left(\frac{\lambda_j}{\lambda_{j+1}}\right)\ge\mathcal{O}(10^1)$}.
...
problem with uncertain \rebut{$\log(A_i)$ values as discussed earlier}.
...
the resulting normalized eigenvalue spectrum \rebut{by plotting $\log(\lambda_i/\lambda_0)$} ($i = 1,\ldots,19$) corresponding to $N$~=~1000 and the same quantity corresponding to
...
$\lambda_1$ is roughly \rebut{an order of magnitude} larger than $\lambda_2$, we expect 
a 1-dimensional active subspace \rebut{to reasonably approximate the uncertainty
...
%\bigskip
%
\rebut{The quantity, $\max(\delta \hat{\mat{W}}_{1,j}^{(r)})$ is plotted in
...
The legend provides the number of samples, $N$ and correspondingly, the
model runs, $M$ required for obtaining a converged dominant eigenvector using the perturbation and
regression strategies. Corresponding value of $N$ and $M$ for the test case are also provided.}}
\label{fig:comp}
\end{center}
\end{figure}
%
\rebut{In Figure~\ref{fig:comp} (left), we compare the
squared values of individual components of the dominant eigenvector obtained using the two approaches
at the end of 8 iterations in each case. Additionally, the dominant eigenvector components obtained
for a test case involving $N$~=~1000 samples using the perturbation approach are also plotted. 
...
H$_2$/O$_2$ reaction in this case, varies
 predominantly in \rebut{a 1-dimensional} active subspace. Hence, 
$f(\bm{\xi})$ can be approximated as $G(\hat{\mat{W}}_1^\top\bm{\xi})$ in
the \rebut{1-dimensional active} subspace. The plot of $G$ versus $\hat{\mat{W}}_1^\top\bm{\xi}$, 
...
\end{figure}
%
\rebut{The dominant eigenvector obtained using perturbation is based on $N$~=~45 samples which requires
$M$~=~900 model runs. For the same amount of computational effort, we can afford $N$~=~900 samples
when using regression. Hence, the SSP from regression is based on 900
points: ($\hat{\mat{W}}_1^\top\bm{\xi}$,$G(\hat{\mat{W}}_1^\top\bm{\xi})$). On the other hand, the SSP
from perturbation is plotted using only 45 points as mentioned earlier.
Nevertheless, the illustrative comparison clearly indicates that the two SSPs
are in excellent agreement. Moreover, it is interesting to note
that the response in ignition
delay based on the considered probability distributions for $\log(A_i)$ although non-linear, can be approximated by
a 1-dimensional active subspace.}

We further estimate
the normalized activity scores for individual uncertain inputs ($\tilde{\nu}_{i,r}$; $r$=1 \rebut{since a
...
\caption{\rebut{Left: A bar-graph of normalized activity scores ($\tilde{\nu}_{i,r}$'s) 
...
associated with the ignition delay is largely due to the uncertainty in $A_9$ \rebut{while $A_1$, $A_{15}$, and
...
approaches in this case. Since the regression approach requires fewer model evaluations for
gradient estimation, it might be a preferred approach for this setting.
In the following section, we shift our focus to 
the higher-dimensional H$_2$/O$_2$ reaction kinetics application \rebut{wherein the activation
