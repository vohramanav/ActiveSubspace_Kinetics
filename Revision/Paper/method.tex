...
%\bigskip
%
\rebut{The quantity, $\max(\delta \hat{\mat{W}}_{1,j}^{(r)})$ defined in Algorithm~\ref{alg:grad} was
...
model runs were required to obtained a converged active subspace in both cases. 
...
\begin{figure}[htbp]
 \begin{center}
  \includegraphics[width=0.45\textwidth]{./Figures/eig_conv19D}
\caption{\rebut{An illustrative comparison of individual squared components of the converged dominant
eigenvector obtained using perturbation and regression strategies using $M$~=~900 model runs in each case. 
Additionally, the dominant eigenvector components obtained using $M$~=~20000 model runs (corresponding to 
$N$~=~1000 samples)
in the perturbation strategy (test case), used to assess the accuracy of the two strategies are also plotted.}}
